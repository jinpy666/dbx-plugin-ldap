...
# then restart DBX. Older io.dbx.ldap versions are removed after install
# (pass --keep-old to keep them for rollback).
#
# Usage:
...
#   scripts/install.sh --keep-old         # keep older io.dbx.ldap versions for rollback
#
# Environment:
...
if [ "$REINSTALL" = 1 ]; then
  VERSION_DIR="$PLUGIN_STORE/io.dbx.ldap/versions/$VERSION"
  if [ -d "$VERSION_DIR" ]; then
    echo "==> dev reinstall: removing installed v$VERSION"
    rm -rf "$VERSION_DIR"
    python3 - "$PLUGIN_STORE/io.dbx.ldap/activations" "$VERSION" <<'PY'
...
if [ "$KEEP_OLD" = 0 ]; then
  echo "==> cleaning old io.dbx.ldap versions (keeping v$VERSION)"
...
versions_dir = plugin_store / "io.dbx.ldap" / "versions"
activations_dir = plugin_store / "io.dbx.ldap" / "activations"
...
PY
  ...
  echo "==> keeping old io.dbx.ldap versions (--keep-old)"
fi
